...
func CalcDaYun(pSiZhu *TSiZhu, nSex int) TDaYun {
	...
	for i := 0; i < 12; i++ {
		...
		// 获取干支名称
		ExtractGanZhi2(&dayun.Zhu[i].GanZhi, &dayun.Zhu[i].Gan, &dayun.Zhu[i].Zhi)

		// 年干
		GetShiShenFromGan3(&dayun.Zhu[i].Gan, nDayGan)

		// 藏干
		GetCangGanFromZhi2(&dayun.Zhu[i].Zhi)

		// 地支藏转十神
		for j := 0; j < 3; j++ {
			// 年
			CalcCangGan(nDayGan, &dayun.Zhu[i].Zhi.CangGan[j])
		}
	}
...
func CalcQiYun(pDaYun *TDaYun, dtPreviousJie TDate, dtNextJie TDate, dtSolarDate TDate) {
	...
	if pDaYun.ShunNi {
		// 顺推找下一个节
		nDiffSeconds = GetDiffSeconds(dtSolarDate.Year, dtSolarDate.Month, dtSolarDate.Day, dtSolarDate.Hour, dtSolarDate.Minute, dtSolarDate.Second,
			dtNextJie.Year, dtNextJie.Month, dtNextJie.Day, dtNextJie.Hour, dtNextJie.Minute, dtNextJie.Second)
	} else {
		// 逆推找上一个节
		nDiffSeconds = GetDiffSeconds(dtPreviousJie.Year, dtPreviousJie.Month, dtPreviousJie.Day, dtPreviousJie.Hour, dtPreviousJie.Minute, dtPreviousJie.Second,
	...
	// 通过计算时间来得到
	pDaYun.QiYun = GetDiffDate2(dtSolarDate, nDiffSeconds)
}
