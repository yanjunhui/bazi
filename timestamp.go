...
// 获取64位时间戳
func GetLunar64TimeStamp(nYear, nMonth, nDay, nHour, nMinute, nSecond int) int64 {
	var nAllDays = GetLunarAllDays(nYear, nMonth, nDay) // 先获取公元原点的日数
	var Result int64 = 0
	Result = int64(nAllDays)
	Result *= 24 * 60 * 60 // 天数换成秒
	//再计算出秒数
...
// 从日期计算时间戳
func GetLunar64TimeStampFromDate(dt TDate) int64 {
	return GetLunar64TimeStamp(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, dt.Second)
}

// 从64位时间戳反推日期
func GetLunarDateFrom64TimeStamp(nTimeStamp int64) TDate {
	var dt TDate
	// 计算年份
	dt.Year = GetLunarYearFrom64TimeStamp(nTimeStamp)
	// 计算月份, 和剩余的时间零头
	dt.Month, nTimeStamp = GetLunarMonthFrom64TimeStamp(nTimeStamp, dt.Year)
...
// 从64位时间戳反推年
func GetLunarYearFrom64TimeStamp(nTimeStamp int64) int {
	// 准备进行二分法
	var nLow int = 1900
	var nHigh int = 2100
	...
	for {
		...
		// 拿到中间年的数据
		var v int64 = GetLunar64TimeStamp(nMid, 1, 1, 0, 0, 0)
...
// 从64位时间戳反推月, 返回月份和剩余时间戳
func GetLunarMonthFrom64TimeStamp(nTimeStamp int64, nYear int) (int, int64) {
	var v int64 = GetLunar64TimeStamp(nYear, 1, 1, 0, 0, 0)
	// 拿到当年的时间戳
	nTimeStamp -= v
	var ONE_DAY_SECOND int = 24 * 60 * 60
	// 这里开始特殊处理
	for i := 1; i <= 13; i++ {
		if int(nTimeStamp) < GetLunarMonthDays(nYear, i)*ONE_DAY_SECOND {
			return i, nTimeStamp
		}
		nTimeStamp -= int64(GetLunarMonthDays(nYear, i) * ONE_DAY_SECOND)
	}
